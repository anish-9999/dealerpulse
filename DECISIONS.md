...
The old "X leads waited 48h+" was a single count with a few names. Replaced with three-bucket distribution (0-24h, 24-48h, 48h+) showing the responsiveness shape at a glance, plus a capped top-10 worst offenders list with full context (model, source, status). This prevented ~186 individual entries from overwhelming the UI.
